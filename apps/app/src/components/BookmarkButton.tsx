...
export function BookmarkButton({ userId, question }: BookmarkButtonProps) {
  ...
  if (!userId) {
    // Previously this rendered nothing, so bookmarking was invisible to
    // signed-out visitors rather than being an incentive to sign up. That is
    // consistent with only 45 bookmarks against 2,994 attempts.
    return (
      <a
        href="/signup"
        title="Sign in to save questions"
        aria-label="Sign in to save this question"
        className="p-2 rounded-lg text-[#666666] hover:text-[#a1a1a1] transition-colors"
      >
  ...
  return (
    <div className="relative">
      <button
        aria-label={bookmarkId ? 'Remove bookmark' : 'Bookmark this question'}
