...
/**
 * OpenAI GPT-4o-mini Parallel SEO Content Generator
 *
 * Generates SEO content for subtopics using GPT-4o-mini
 * with parallel processing for speed and cost efficiency.
 *
 * Cost estimate: ~$1.15 for 1000 subtopics
 *
 * Usage:
 *   npx tsx scripts/generate-seo-openai-parallel.ts
 *   npx tsx scripts/generate-seo-openai-parallel.ts --batch 100 --parallel 10
 *   npx tsx scripts/generate-seo-openai-parallel.ts --dry-run
 */
...
async function processBatch(
  ...
  const results = await Promise.allSettled(
    batch.map(async (subtopic, i) => {
      ...
      try {
        const content = await withRetry(() => generateContent(openai, subtopic));
...
Usage: npx tsx scripts/generate-seo-openai-parallel.ts [options]
...
Examples:
  npx tsx scripts/generate-seo-openai-parallel.ts
  npx tsx scripts/generate-seo-openai-parallel.ts --batch 500 --parallel 15
  npx tsx scripts/generate-seo-openai-parallel.ts --level gcse --subject maths
        `);
        process.exit(0);
    }
  }
...
async function main() {
  ...
  console.log('OpenAI GPT-4o-mini Parallel SEO Content Generator');
  console.log('='.repeat(60));
  ...
  if (args.dryRun) {
    ...
    const estimatedCost = (toProcess.length * 0.00115).toFixed(2);
    console.log(`\nWould process: ${toProcess.length} subtopics (before filtering existing)`);
    console.log(`Estimated max cost: ~$${estimatedCost} (GPT-4o-mini)`);
  ...
  // Estimate cost
  const estimatedCost = (toProcess.length * 0.00115).toFixed(2);
  console.log(`Estimated cost: ~$${estimatedCost} (GPT-4o-mini)`);
